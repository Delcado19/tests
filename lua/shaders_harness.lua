-- Run with: REPO_ROOT=/path/to/HyDE sh tests/run.sh shaders
...
write(dir..'allunsafe.frag', '#version 300 es\n#define SHADER_NAME `$"\\\nvoid main() {}\n')
write(dir..'danger`$"file.frag', '#version 300 es\nvoid main() {}\n')
write(dir..'include.frag', '// !source=missing.inc\nvoid main() {}\n')
write(dir..'missing.frag', 'void main() {}\n')
write(dir..'unsafe-icon.frag', '#define SHADER_ICON evil`id`$(id)"\\bad\nvoid main() {}\n')
...
local ok, err = xpcall(function()
    ...
    assert(not sh.apply(nil)); unchanged(before)
    assert(not sh.set(nil)); unchanged(before)
    for _, name in ipairs({'invalid','duplicate','include','unknown', '', '../static', 'STATIC-invalid'}) do
    ...
    assert(not sh.find('unsafe-icon').icon:find('[%c`$"\\]'), 'shell-unsafe icon reached the menu entry')
    local emptied = sh.find('allunsafe')
    assert(emptied and emptied.name == 'allunsafe', 'a name that sanitizes to empty should fall back to the filename')
    local dangerfile = sh.find('danger`$"file')
    assert(dangerfile and dangerfile.name ~= '' and not dangerfile.name:find('[%c`$"\\]'), 'a shell-unsafe filename used as the fallback name must be sanitized too')
    for _, damage in ipairs({-1, 3, '2', false}) do
        current.damage=damage
        local count=calls
        local result, err=sh.set('static')
        assert(not result and err and calls==count, 'invalid compositor damage value was accepted')
        current.damage=before.runtime.damage; unchanged(before)
    end
    ...
    menu=function()
        ...
        assert(sh.preview('disable',nil))
        assert(sh.preview('disable','wrong-token'))
        assert(not sh.preview('unknown',stale))
        assert(not sh.preview('',stale))
        assert(not sh.preview('invalid',stale))
        assert(calls==previous_calls); unchanged(before)
    ...
    for _, answer in ipairs({'', 'unknown'}) do
        menu=function() assert(sh.preview('comments',token())); return answer end
        assert(not sh.select()); unchanged(before)
    end
    menu=function() assert(sh.preview('comments',token())); error('simulated menu failure') end
    local result, err=sh.select()
    assert(not result and err:find('simulated menu failure',1,true)); unchanged(before)
    print('PASS: empty/unknown selections and menu errors restore the committed state')
